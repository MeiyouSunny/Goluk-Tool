...
local function jsonencode (v)
  ...
  if vtype=='table' then
    ...
    if bArray then
      for i = 1,maxCount do
        table.insert(rval, jsonencode(v[i]))
      end
    else	-- An object, not an array
      for i,j in pairs(v) do
        if isEncodable(i) and isEncodable(j) then
          table.insert(rval, '"' .. encodeString(i) .. '":' .. jsonencode(j))
        end
      end
    end
    if bArray then
  ...
  assert(false,'jsonencode attempt to jsonencode unsupported type ' .. vtype .. ':' .. tostring(v))
end
...
--[[	
	...
	function jsondecode (js_string)
		...
		local function next_token (tok)
			...
			--error("unexpected termination of JSON while looking for "..tok.name) 先注释掉 2012-7-28-kxy
		end
		...
		function read_array (o,i)
			...
			-- loop until ...
			while true do
			local t = next_token(tt_array_seperator) --kxy--2012-07-03
			if t == true then
				return o
			else
...
--table 转 json
createmodule(interface,"encode",function (v)
	return jsonencode(v);
end)

--json 转 table
createmodule(interface,"decode",function (js_string)
	return jsondecode (js_string);
end)

--保留js null
createmodule(interface,"null",null)

tiros.json = readOnly(interface)
