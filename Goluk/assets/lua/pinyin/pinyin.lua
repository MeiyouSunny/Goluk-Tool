...
require"lua/pinyin/bit"

local _pinyin_table_ = {
...
local function hz2py_str_copy(to, from, count)
	...
	if (count<#from) then
		to = to..string.sub(from,1,count)
		from = string.sub(from,count+1,-1)
	end
	return to, from
end
...
local function is_utf8_string(utf)
    ...
    for i=1,length do
        if ( check_sub == 0 ) then
	    if ( tiros.tirosbit.bit.brshift(string.byte(utf,i), 7) ~= 0 ) then
		    if ( tiros.tirosbit.bit.band(string.byte(utf,i), 0xE0) == 0xC0 ) then
		        check_sub = 1
		    elseif ( tiros.tirosbit.bit.band(string.byte(utf,i), 0xF0) == 0xE0 ) then
		        check_sub = 2
		    elseif ( tiros.tirosbit.bit.band(string.byte(utf,i), 0xFC) == 0xF8 ) then
		        check_sub = 4
		    elseif ( tiros.tirosbit.bit.band(string.byte(utf,i), 0xFE) == 0xFC ) then
		        check_sub = 5
		    else
		        return false
		    end
	   ...
	   if ( tiros.tirosbit.bit.band(string.byte(utf,i), 0xC0) ~= 0x80 ) then
                return false
           end
...
--将utf8编码的字符串中的汉字解成拼音
-- input 输入
-- is_first_letter_only 是否只输出拼音首字母
-- is_polyphone_support 是否输出多音字
-- is_add_blank 是否在拼音之间追加空格
-- is_convert_double_char 是否转换全角字符为半角字符

local function utf8topinyin(
        input,
        is_add_blank,
	is_first_letter_only,
	is_polyphone_support,
	is_convert_double_char
        )
	...
	local out = ""
	local out_start_flag = out
	local uni
	local splitpos = nil

	while #utf~=0 do
		--is_convert_flag = false
		if ( tiros.tirosbit.bit.brshift(string.byte(utf,1), 7) == 0 ) then
			--is_convert_flag = true
			out,utf = hz2py_str_copy(out, utf, 1)
		elseif ( tiros.tirosbit.bit.band(string.byte(utf,1), 0xE0) == 0xC0 ) then   --两个字节
			--is_convert_flag = true
			out,utf = hz2py_str_copy(out, utf, 2)
		elseif ( tiros.tirosbit.bit.band(string.byte(utf,1), 0xF0) == 0xE0 ) then   --三个字节

			if #utf>=3 then
				uni = tiros.tirosbit.bit.bor( tiros.tirosbit.bit.bor(tiros.tirosbit.bit.blshift( tiros.tirosbit.bit.band(string.byte(utf, 1), 0x0F), 12 )
					, tiros.tirosbit.bit.blshift( tiros.tirosbit.bit.band(string.byte(utf, 2), 0x3F), 6 ))
					, tiros.tirosbit.bit.band(string.byte(utf, 3), 0x3F) )
				if ( (uni>19967) and (uni < 40870) ) then
					if ( is_add_blank and (#out>#out_start_flag) and (string.sub(out, -1, -1)~="") 
							and (string.sub(out, -1, -1)~="\r") and (string.sub(out, -1, -1)~="\n") 
							and (string.sub(out, -1, -1)~="\t") ) then
						out = out.." "
					end

					py_tmp = _pinyin_table_[uni-19968+1]

					_tmp = py_tmp
					_tmp2 = py_tmp2

					if is_first_letter_only then
						_tmp2 = _tmp2..string.sub(_tmp,1,1)
						_tmp = string.sub(_tmp,2,-1)

						while(#_tmp~=0) do
						    if ( (string.sub(_tmp,1,1) == '|') or (string.sub(_tmp2,-1,-1) == '|') ) then
								_tmp2 = _tmp2..string.sub(_tmp,1,1)
						    end
						    _tmp = string.sub(_tmp,2,-1)
						end					   
					...
					if not is_polyphone_support then						
							splitpos = string.find(_tmp2, '|')
							if nil ~= splitpos then
								 _tmp2 = string.sub(_tmp2,1,splitpos-1)
							end					
					end

					out = out.._tmp2
					
					if is_add_blank then
						out = out.." "
					end

					utf = string.sub(utf,4,-1)						
							
				elseif ( is_convert_double_char and (uni>65280) and (uni<65375) ) then 
					out = out..string.char(uni-65248)
					utf = string.sub(utf,4,-1)						
			      	elseif ( is_convert_double_char and  (uni==12288) ) then
					out = out..string.char(32)
					utf = string.sub(utf,4,-1)
				else
					out,utf = hz2py_str_copy(out, utf, 3)
				end										
			else
				out,utf = hz2py_str_copy(out, utf, 3)
			end
		elseif ( tiros.tirosbit.bit.band(string.byte(utf,1), 0xF8) == 0xF0 ) then   --四个字节
			out,utf = hz2py_str_copy(out, utf, 4)
		elseif ( tiros.tirosbit.bit.band(string.byte(utf,1), 0xFC) == 0xF8 ) then   --五个字节
			out,utf = hz2py_str_copy(out, utf, 5)
		elseif ( tiros.tirosbit.bit.band(string.byte(utf,1), 0xFE) == 0xFC ) then   --六个字节
			out,utf = hz2py_str_copy(out, utf, 6)
		else 
			out,utf = hz2py_str_copy(out, utf, 1)
			break
		end
...
local interface = {}
-------------------------全局函数，对外接口-----------------------
createmodule(interface,"utf8_to_pinyin", function ( input,is_add_blank,is_first_letter_only,is_polyphone_support,is_convert_double_char)
	return  utf8topinyin(input,is_add_blank,is_first_letter_only,is_polyphone_support,is_convert_double_char)
end)
-----------------------------------------------------------
tiros.pinyin = readOnly(interface)
--print( is_utf8_string(string.char(97)))
--print("out=",tiros.pinyin.utf8_to_pinyin("中jslkjskdf;klfdkse98士大夫士大夫萨达玫瑰红海茫茫号码和34583;sdfg98972367sdfsdfjsdfjsldferutoerutoerue适当放宽适当放宽士大夫士大夫士大夫华，。！&#×￥３４５３４５３４５（#！@）？人们共和国",false,false,false,true))
