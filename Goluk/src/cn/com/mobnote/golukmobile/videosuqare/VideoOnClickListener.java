...
public class VideoOnClickListener implements OnClickListener {
	...
	public VideoOnClickListener(List<VideoSquareInfo> _mVideoSquareListData, ViewHolder _mViewHolder,
			HashMap<String, DWMediaPlayer> _mDWMediaPlayerList, VideoSquareInfo _mVideoSquareInfo, Context context,
			int platform) {
	...
	public void onClick(View arg0) {
		if ("2".equals(mVideoSquareInfo.mVideoEntity.type)) {
			if (1 == form) {
				...
				// VideoSquarePlayActivity a = (VideoSquarePlayActivity)
				// mContext;
				// Intent intent = new Intent(a, VideoPlayerView.class);
				// intent.putExtra("from", "suqare");
				// intent.putExtra("image",
				// mVideoSquareInfo.mVideoEntity.picture);
				// intent.putExtra("playUrl",
				// mVideoSquareInfo.mVideoEntity.ondemandwebaddress);
				// uploadPlayer(mVideoSquareInfo.mVideoEntity.videoid, "1",
				// "1");// 上报播放次数
				// a.startActivity(intent);
			}
