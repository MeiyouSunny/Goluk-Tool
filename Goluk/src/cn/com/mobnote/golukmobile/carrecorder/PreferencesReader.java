...
public class PreferencesReader {
	...
	VideoConfiguration vcLocal = getLocal(c, mdpi_local, mframe_local,
		mma_local);
	m_crcConfig.setLocalVideoConfig(vcLocal); // 本地视频配置
	Log.d(TAG, "local config:" + vcLocal.toString());

	String strRtmpLive = sp.getString(PreferencesReader.URL_LIVE,
		c.getString(R.string.default_live_url));
	...
	int maxSize = Integer.parseInt(c.getResources().getStringArray(
		R.array.list_maxsize_key_local)[mIndex_MaxSize]);
	m_crcConfig.setMaxSize(maxSize); // 控制剩余空间可录制的视频段数
	m_crcConfig.setItemTime(mItmeTime * 60000); // 分段时每段的时长，实际配置单位为ms
	m_crcConfig.setLiveEnableHW(Integer.parseInt(sp.getString(
		PreferencesReader.HW_LIVE, "0")) == 1);
	// 直播缓冲时间
    ...
    private VideoConfiguration getLive(Context c, int dpi_live, int frame_live,
	    int ma_live) {
...
//	liveVideoConfig.setVideoSize(width, height);
    ...
    private VideoConfiguration getLocal(Context c, int dpi_local,
	    int frame_local, int ma_local) {
	VideoConfiguration localVideoConfig = new VideoConfiguration();

	localVideoConfig.setVideoFrameRate(c.getResources().getIntArray(
		R.array.list_frame_source_local)[frame_local]);
	localVideoConfig.setVideoFrameRate(c.getResources().getIntArray(
		R.array.list_frame_source_local)[frame_local]);
	// 1M = 1024*1024 bytes
	localVideoConfig.setVideoEncodingBitRate(c.getResources().getIntArray(
		R.array.list_ma_source_local)[ma_local] * 1024 * 1024);
	int width = 1920;
	int height = 1080;
	if (dpi_local == 1) { // 720p
	    width = 1280;
