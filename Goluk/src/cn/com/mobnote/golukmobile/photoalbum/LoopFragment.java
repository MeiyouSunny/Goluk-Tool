		...
		mStickyListHeadersListView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView arg0, int scrollState) {
				...
				case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
					...
					GolukDebugUtils.e("",
							"YYYYYY=====SCROLL_STATE_IDLE====11111111111=");
		...
		mStickyListHeadersListView.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						if (arg2 < mDoubleDataList.size()) {
							RelativeLayout mTMLayout1 = (RelativeLayout) arg1
									.findViewById(R.id.mTMLayout1);
							RelativeLayout mTMLayout2 = (RelativeLayout) arg1
...
//			this.getContext().startActivity(intent);
	...
	private void addFooterView() {
		if (!addFooter) {
			addFooter = true;
			mBottomLoadingView = (RelativeLayout) LayoutInflater.from(
		...
		case ENetTransEvent_IPC_VDCP_CommandResp:
			if (IPC_VDCP_Msg_Query == msg && mFragmentAlbum != null && mFragmentAlbum.mCurrentType == 3) {
				...
				if (RESULE_SUCESS == param1) {
					...
					ArrayList<VideoInfo> fileList = IpcDataParser
							.parseVideoListData((String) param2);
					...
					try {
						...
						if (null != json) {
							...
							if (tag.contains("IPC_IMAGE")) {
								int type = IpcDataParser
										.parseVideoFileType(filename);
								if (type != IPCManagerFn.TYPE_CIRCULATE) {
									return;
								}
