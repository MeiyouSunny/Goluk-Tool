...
public class ThirdPlatformLoginUtil {
	...
	public void addWXPlatform() {
		...
		UMWXHandler wxHandler = new UMWXHandler(mContext, GolukConfig.WX_APPID,
				GolukConfig.WX_APPSECRET);
	...
	public void login(SHARE_MEDIA platform) {
		...
		mController.doOauthVerify(mContext, platform, new UMAuthListener() {
			...
			public void onStart(SHARE_MEDIA platform) {
				GolukDebugUtils.d(TAG, "weixin login onStart");
			}

			@Override
			public void onError(SocializeException e, SHARE_MEDIA platform) {
				e.printStackTrace();
				GolukDebugUtils.d(TAG, "weixin login onError");
				if (mListener != null) {
			...
			public void onComplete(Bundle value, SHARE_MEDIA platform) {
				GolukDebugUtils.d(TAG, "weixin login onComplete");
				// 获取uid
			...
			public void onCancel(SHARE_MEDIA platform) {
				GolukDebugUtils.d(TAG, "weixin login onCancel");
				if (mListener != null) {
					mListener.getUserInfo(false, null, null);
				}
			}
	...
	public void getUserInfo(SHARE_MEDIA platform) {
		mController.getPlatformInfo(mContext, platform, new UMDataListener() {
			...
			@Override
			public void onStart() {
				GolukDebugUtils.d(TAG, "weixin getUserInfo onStart");
			}

			@Override
			public void onComplete(int status, Map<String, Object> info) {
				GolukDebugUtils.d(TAG, "weixin getUserInfo onComplete");
				if (info != null) {
					...
					if (mListener != null) {
						...
						GolukFileUtils.saveString(
								GolukFileUtils.THIRD_USER_INFO, infoStr);
						GolukFileUtils.saveString(
								GolukFileUtils.LOGIN_PLATFORM, "weixin");
					}
				}
			}
		});
	...
	public void logout(SHARE_MEDIA platform) {
		mController.deleteOauth(mContext, platform,
			...
			@Override
			public void onStart() {
				GolukDebugUtils.d(TAG, "weixin logout onStart");
			}

			@Override
			public void onComplete(int status, SocializeEntity entity) {
				GolukDebugUtils.d(TAG, "weixin logout onComplete");
			}
