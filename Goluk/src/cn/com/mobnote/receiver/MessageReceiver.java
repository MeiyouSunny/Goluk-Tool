...
public class MessageReceiver extends XGPushBaseReceiver {
	...
	public void onUnregisterResult(Context context, int errorCode) {
		Log.e("", "XD----MessageReceiver---onUnregisterResult- errorCode: " + errorCode);
		if (context == null) {
			return;
		}
		String text = "";
	...
	public void onTextMessage(Context context, XGPushTextMessage message) {
		...
		dealTextMsg(context, customContent);
	}
	 ...
	 * @author jyf
	 */
	private void dealTextMsg(Context context, String json) {
		...
		if (GolukApplication.getInstance() != null && !GolukApplication.getInstance().isExit()) {
			// 程序内显示框
			GolukNotification.getInstance().showAppInnerPush(bean);
		} else {
			// 程序外通知
			GolukNotification.getInstance().showNotify(context, bean);
		}
	}
