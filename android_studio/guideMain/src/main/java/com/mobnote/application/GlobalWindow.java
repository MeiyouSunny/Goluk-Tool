...
public class GlobalWindow implements View.OnClickListener {
    ...
    public void createVideoUploadWindow(String promptText) {
        ...
        boolean hasPermission = FloatWindowPermissionUtil.judgePermission(mContext);
        if (!hasPermission) {
            if (!toastShowed) {
                Toast.makeText(mContext, mContext.getString(R.string.str_system_window_not_allowed), Toast.LENGTH_SHORT).show();
                toastShowed = true;
            }
            return;
        }
