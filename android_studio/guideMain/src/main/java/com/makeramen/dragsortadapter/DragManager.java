...
final class DragManager implements View.OnDragListener {
	...
	public boolean onDrag(View v, DragEvent event) {
		if (v != recyclerViewRef.get()
				|| !(event.getLocalState() instanceof DragInfo)) {
			return false;
		}
		...
		case DragEvent.ACTION_DRAG_STARTED:
			draggingId = itemId;
			adapter.notifyItemChanged(recyclerView.findViewHolderForItemId(
					itemId).getAdapterPosition());
			break;
		...
		case DragEvent.ACTION_DRAG_LOCATION:
			...
			View child = recyclerView.findChildViewUnder(event.getX(),
					event.getY());
			if (child != null) {
				toPosition = recyclerView.getChildViewHolder(child)
						.getAdapterPosition();
			}

			if (toPosition >= 0 && fromPosition != toPosition) {
				RecyclerView.ItemAnimator animator = recyclerView
						.getItemAnimator();

				boolean scheduleNextMove = nextMoveTouchPoint.equals(MIN_VALUE,
						MIN_VALUE);
				...
				if (scheduleNextMove)
					animator.isRunning(new RecyclerView.ItemAnimator.ItemAnimatorFinishedListener() {
						...
						public void onAnimationsFinished() {
							...
							final int fromPosition = adapter
									.getPositionForId(itemId);

							View child = recyclerView.findChildViewUnder(
									nextMoveTouchPoint.x, nextMoveTouchPoint.y);
							if (child != null) {
								final int toPosition = recyclerView
					...
					.isRunning(
							new RecyclerView.ItemAnimator.ItemAnimatorFinishedListener() {
								...
								public void onAnimationsFinished() {
									...
									if (vh != null
											&& vh.getAdapterPosition() != position) {
										// if positions don't match, there's
										// still an outstanding move animation
										// so we try to reschedule the
										// notifyItemChanged until after that
										recyclerView.post(new Runnable() {
											@Override
											public void run() {
												recyclerView
