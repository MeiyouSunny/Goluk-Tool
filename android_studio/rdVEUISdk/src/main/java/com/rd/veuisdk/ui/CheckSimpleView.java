...
public class CheckSimpleView extends SimpleDraweeView {
    ...
    private Drawable mDrawableBelong;
    ...
    public CheckSimpleView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mIconWidth = (int) getResources().getDimension(R.dimen.dimen_30);
        mIconHeight = (int) getResources().getDimension(R.dimen.dimen_30);

        mReady = true;
        mDrawableSelected = getResources().getDrawable(R.drawable.bottom_item_checked);
        mDrawableBelong = getResources().getDrawable(R.drawable.prompt_point);
        if (mSetupPending) {
            setup();
            mSetupPending = false;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int selectLeft = getWidth() - mDrawableSelected.getIntrinsicWidth();
        int selectTop = getHeight() - mDrawableSelected.getIntrinsicHeight();
        mBitmap = getBitmapFromDrawable(getTopLevelDrawable());
        if (mBitmap != null) {
            setup();
        }

        canvas.drawRect(0, 0, mIconWidth, mIconHeight, mBitmapPaint);
        ...
        if (isLoading && mDrawableLoading != null) {
            ...
            if (isChecked()) {
                ...
                mDrawableBelong.setBounds(
                        getWidth() - mDrawableBelong.getIntrinsicWidth(),
                        getHeight() - mDrawableBelong.getIntrinsicHeight(),
                        getWidth(), getHeight());
                mDrawableBelong.draw(canvas);
            }
        }
    }
