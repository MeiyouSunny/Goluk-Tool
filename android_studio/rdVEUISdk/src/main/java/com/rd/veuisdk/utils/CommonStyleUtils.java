...
public class CommonStyleUtils {
    ...
    //当前的播放器视频size
    private static double previewWidth = 640.0, previewHeight = 640.0;

    //字幕特效默认配置文件是按照640*360 大小设计的，所以为了适配任意的分辨率，需要转化为0~1.0f , 再根据目标宽高
    private static final float PWIDTH = 640f, PHEIGHT = 640f;
        ...
        if (!TextUtils.isEmpty(content)) {
            try {
                ...
                if (json.optInt("ver", 0) >= 1) {
                    ...
                    if (!info.isSetSizeW()) {
                        //默认时，字幕特效的显示宽高 单位:像素
                        info.w = json.getDouble("width");
                        info.h = json.getDouble("height");
                    }
                    ...
                    info.unLoop = json.optInt("unLoop", 0) == 1;
                    if (info.isApng) { //贴纸支持apng 190710
                        ...
                        if (info.isSub()) { //字幕
                            SubUtils.webp2png(info);
                        } else {//贴纸
                            StickerUtils.webp2png(info);
                        }
                        ...
                        if (null != config) {
                            ...
                            if (null != jarr && jarr.length() > 0) {
                                ...
                                frameInfo = new FrameInfo();
                                frameInfo.time = 0;
                                frameInfo.pic = tmpFilePath + "0.png";
                                info.frameArray.put(frameInfo.time, frameInfo);
                            }
                        }
                    }
                    ...
                    if (info.isSetSizeW()) {
                        ...
                        if (info.frameArray.size() >= 1 && FileUtils.isExist(info.frameArray.valueAt(0).pic)) {
                            ...
                            float maxOutXMarginDesign = (float) (minXMargin * PWIDTH);
                            scaleW = (float) (((maxOutXMargin) / maxOutXMarginDesign));

                            //中心点在Y轴上距离上下最近的一边的距离
                            float minYMargin = Math.min((1 - info.centerxy[1]), info.centerxy[1]);
                            float maxOutYMargin = (float) (minYMargin * previewHeight) - 30;
                            float maxOutYMarginDesign = (float) (minYMargin * PHEIGHT);
                            float defaultMaxScaleH = (float) ((maxOutYMargin / maxOutYMarginDesign));
...
//                        Log.e(TAG, "getConfig: " + info.code + ">>" + scaleW + " >" + defaultMaxScaleH);
                        ...
                        float minScaleH = (float) (60.0f / info.h);

                        info.disf = Math.max(CaptionObject.MIN_SCALE, Math.min(CaptionObject.MAX_SCALE, Math.max(minScaleH, scaleW)));

//                    Log.e(TAG, "getConfig: " + info.code + " " + info.disf + "   " + info.w + "    (" + previewWidth + " / " + PWIDTH + ")");
...
//                Log.e(TAG, "getConfig: " + info);
    ...
    /**
     * 退出字幕编辑时，判断数据是否有变化，提示是否退出
     *
     * @param templist
     * @param savelist
     * @return
     */
    public static boolean isEquals(ArrayList<WordInfo> templist, ArrayList<WordInfo> savelist) {
     ...
     * @return
     */
    public static boolean isEqualsMosaic(ArrayList<MOInfo> templist, ArrayList<MOInfo> savelist) {
    ...
    /**
     * 退出特效编辑时，判断数据是否有变化，提示是否退出
     *
     * @param templist
     * @param savelist
     * @return
     */
    public static boolean isEqualsSp(ArrayList<StickerInfo> templist,
                                     ArrayList<StickerInfo> savelist) {
