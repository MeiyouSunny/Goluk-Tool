...
public class GraphicsHelper {
     ...
     *
     * @param src
     * @param cornersRadius 圆角大小 单位:px
     * @return
     */
    public static Bitmap createSquareRoundedCornerBitmap(Bitmap src, int cornersRadius) {
        int srcW = src.getWidth();
        int srcH = src.getHeight();
        Bitmap output = Bitmap.createBitmap(srcW, srcH
                , Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
...
//            Log.e(TAG, "getBitmap: " + width + "*" + height + " rect:  " + rect);
     ...
     * @param bitmap
     * @param radius
     * @param borderWeight 环形宽
     * @param borderColor  环形背景color
     * @param bgColor      圆形bmp背景图
     * @param ischeck      是否被选中
     * @param progress     下载进度默认100
     */
    public static void drawRoundedCornerBitmap(Canvas canvas, int width,
                                               int height, Bitmap bitmap, int radius, int borderWeight,
        ...
        if (ischeck) {// 被选中的背景空心圆
            ...
            paint.setStrokeWidth(borderWeight);
     ...
     * @param bgColor       背景色
     * @param cornersRadius 圆角大小
     */
    public static void drawRoundedSquareCornerBitmap(Canvas canvas, int width,
                                                     int height, Bitmap bitmap, int borderWidth,
                                                     int borderColor, int bgColor, int cornersRadius) {
        Bitmap zoomBitmap = zoomBitmap(bitmap, width, height);
        //矩形带圆角
        bitmap = createSquareRoundedCornerBitmap(zoomBitmap, cornersRadius);
        zoomBitmap.recycle();
        RectF dst = new RectF(borderWidth, borderWidth, width - borderWidth,
                height - borderWidth);
        Paint paint = new Paint();
        paint.setAntiAlias(true);// 设置去锯齿
        paint.setColor(bgColor);
