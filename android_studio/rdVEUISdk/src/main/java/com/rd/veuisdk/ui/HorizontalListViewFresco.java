...
public class HorizontalListViewFresco extends HorizontalScrollView implements Rotatable {
    private final String TAG = "HorizontalListViewFresco";
    private int mColorNormal, mColorSelected;

    public HorizontalListViewFresco(Context context, AttributeSet attrs) {
        super(context, attrs);

        mLayoutInflater = (LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);

        mSaCameraFilter = new SparseArray<>();

        mListViewItemLayoutId = R.layout.fresco_list_item;
    ...
    private class ListViewItem implements Rotatable {
        ...
        private void onImp(int nItemId) {
            ...
            mItemId = nItemId;
            mItemImageView = (ExtCircleSimpleDraweeView) mListViewItem
            ...
            mListViewItem.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    onListItemClick(HorizontalListViewFresco.ListViewItem.this);
                }
            });
//            if (mItemId == 0) {
...
//            }
        ...
        public ListViewItem(int nItemId, String localPic) {
            onImp(nItemId);
            SimpleDraweeViewUtils.setCover(mItemImageView, localPic);
        }
...
//            mItemImageView.setOrientation(orientation);
         ...
         * @param url
         */
        public void setItemResource(String url) {
            SimpleDraweeViewUtils.setCover(mItemImageView, url);
        }
    ...
    public void addListItem(int nItemId, String picLocal, String strCaption
        ...
        ListViewItem item = mSaCameraFilter.get(nItemId);
        if (null == item) {
            item = new ListViewItem(nItemId, picLocal);
            mSaCameraFilter.put(nItemId, item);
            item.setItemCaption(strCaption);
        } else {
            item.setItemResource(picLocal);
            item.setItemCaption(strCaption);
        }
    }
...
//        Log.e(TAG, "selectListItem: " + nItemId + ">>" + user + (null != mLastSelectedItem));
        ListViewItem clickItem = mSaCameraFilter.get(nItemId);
    ...
    public void setOrientation(boolean isVer) {
        if (CoreUtils.hasJELLY_BEAN_MR2()) {
            if (isVer) {
                ...
                mListViewItemLayoutId = R.layout.fresco_list_item_land;
            }
        } else {
            //4.2 以下，滤镜图标为正方形
            if (isVer) {
