...
public class BoundedRect {
    ...
    public BoundedRect(float rotation, Rect outerRect, Rect innerRect) {
        mRotation = rotation;
        mOuterRect = new RectF(outerRect);
    ...
    public void moveInner(float dx, float dy) {
        ...
        float[] outerCorners = CropMath.getCornersFromRect(mOuterRect);
        ...
        for (int i = 0; i < translatedInnerCorners.length; i += 2) {
            ...
            if (!CropMath.inclusiveContains(mOuterRect, correctedInnerX,
                ...
                CropMath.getEdgePoints(mOuterRect, correctionVec);
    ...
    public void resizeInner(RectF newInner) {
        ...
        for (int i = 0; i < newInnerCorners.length; i += 2) {
            ...
            m0.mapPoints(c0);
            if (!CropMath.inclusiveContains(mOuterRect, c0[0], c0[1])) {
                ...
                float[] pathOfCorner = {newInnerCorners[i],
                        ...
                        oldInnerCorners[i + 1]};
                float[] p = GeometryMathUtils.lineIntersect(pathOfCorner,
                        outerSide);
                if (p == null) {
    ...
    public void fixedAspectResizeInner(RectF newInner) {
        ...
        float[] corners = CropMath.getCornersFromRect(mOuterRect);
        ...
        // find fixed corner
        int fixed = -1;
        if (mInnerRect.top == newInner.top) {
            if (mInnerRect.left == newInner.left)
                fixed = 0; // top left
            else if (mInnerRect.right == newInner.right)
                fixed = 2; // top right
        } else if (mInnerRect.bottom == newInner.bottom) {
            if (mInnerRect.right == newInner.right)
                fixed = 4; // bottom right
            else if (mInnerRect.left == newInner.left)
                fixed = 6; // bottom left
        }
        ...
        RectF ret = new RectF(mInnerRect);
        if (fixed == 0) {
    ...
    private boolean isConstrained() {
        for (int i = 0; i < 8; i += 2) {
            if (!CropMath.inclusiveContains(mOuterRect, mInnerRotated[i],
                    mInnerRotated[i + 1]))
                return false;
        }
        return true;
    ...
    private void reconstrain() {
        ...
        CropMath.getEdgePoints(mOuterRect, mInnerRotated);
        Matrix m = getRotMatrix();
    ...
    private Matrix getRotMatrix() {
        Matrix m = new Matrix();
        m.setRotate(mRotation, mOuterRect.centerX(), mOuterRect.centerY());
        return m;
    }
