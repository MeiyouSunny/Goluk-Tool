...
import com.rd.veuisdk.ui.DraggableGridView.onLonglistener;
...
public class DraggableAddGridView extends ViewGroup {
    ...
    // dragging vars
    protected int mDraggedIndex = -1, mLastXPosition = -1, mLastYPosition = -1, mLastTarget = -1;
    private int mXPosition = -1, mYPosition = -1;
            ...
            if (mOldItemCount != mItemCount) {
                ...
                for (int nTmp = 0; nTmp < mItemCount; nTmp++) {
                    if (nTmp < getChildCount()) {
                        if (this.getChildAt(nTmp) instanceof AddViewItem) {
                            continue;
                        }
                        itemView = (DraggableGridViewItem) this.getChildAt(nTmp);
                    } else {
                        itemView = null;
                    }

                    contentView = mRearrageAdapter.getView(nTmp, itemView, this);
                    if (itemView == null) {
                        itemView = new DraggableGridViewItem(this.getContext(), contentView);
                ...
                if (!mHideAddItem) {
                    ...
                    for (int nTmp = 0; nTmp <= mItemCount; nTmp++) {
                        ...
                        if (itemAdd == null) {
                            ...
                            contentView = itemAdd.getContentView();
                        }
    ...
    public int getIndexFromCoor(int x, int y) {
        int col, row;
        if (mOrientation == VERTICAL) {
            col = getColFromCoor(x);
            row = getRowFromCoor(y + mScrollPosition);
            if (col > 3) {
                col = 3;
            }
        } else { // 水平
            if (mHideAddItem) {
                ...
                col = getColFromCoor(x + mScrollPosition);
            }

            row = getRowFromCoor(y);
        }
    ...
    protected int getColFormCoorWithoutAddItem(int coor) {
        coor -= mPadding;
    ...
    protected int getColFromCoor(int coor) {
        ...
        for (int i = 0; mItemWidth >= 0; i++) {
            ...
            coor -= (mItemWidth + mAddButtonWidth);
        }
        return -1;
    }
    ...
    protected int getTargetFromCoor(int x, int y) {
        if (getRowFromCoor(y + mScrollPosition) == -1) {
            return -1;
        }

        int leftPos = getIndexFromCoor(x - (mItemWidth / 4), y);
        int rightPos = getIndexFromCoor(x + (mItemWidth / 4), y);
    ...
    protected Point getCoorFromIndex(int index) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            return new Point((mItemWidth + mAddButtonWidth) * index - mScrollPosition,
                    mPadding);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mXPosition = mLastXPosition;
                mYPosition = mLastYPosition;
                mIsTouching = true;
    ...
    protected void reorderChildren() {
        ...
        if (mDraggedIndex != mLastTarget) {
//             if (mDraggedIndex == 0) {
//             arrAddItemInfo.remove(0);
//             mAddItemListener.newAddItem(mLastTarget - 1);
//             }
//             if (mLastTarget == 0) {
//             arrAddItemInfo.remove(mDraggedIndex - 1);
//             mAddItemListener.newAddItem(0);
//             }
        }
        ...
        mAddItemListener.reorderAddItem(mArrSceneInfo, mDraggedIndex);
    ...
    public void scrollToIndex() {
        mScrollPosition = mChildSize * mDraggedIndex - mXPosition;
        if (mScrollPosition < 0) {
            mScrollPosition = 0;
        }
        if (mScrollPosition > Math.max(getMaxScroll(), 0)) {
            mScrollPosition = Math.max(getMaxScroll(), 0);
        }
        clampScroll();
    }

    public void scrollToIndexWithAddItem() {
        mScrollPosition = (mChildSize + mAddButtonWidth) * mDraggedIndex - mXPosition;
        if (mScrollPosition < 0) {
            mScrollPosition = 0;
        }
        if (mScrollPosition > Math.max(getMaxScroll(), 0)) {
    ...
    protected int getMaxScroll() {
        ...
        if (mOrientation == VERTICAL) {
            ...
            int mColumnCount = mItemCount;
            if (mHideAddItem) {
                max = mColumnCount * (mItemWidth) - getWidth();
            }
//            else if (mHideAddItemWithoutSort) {
//                if (mHideSort) {
//                    max = (mColumnCount + 1) * (mItemWidth) - getWidth();
//                } else {
//                    max = (mColumnCount + 2) * (mItemWidth) - getWidth();
//                }
//            }
            else {
                if (mHideSort) {
    ...
    public void setAddItemSelect(final int index) {
        resetAddItem();
        AddViewItem addItem = (AddViewItem) this.getChildAt(index
                + mItemCount);
        if (addItem != null) {
    ...
    private class AddViewItem extends ViewGroup implements OnClickListener {
        ...
        public void setSelected(boolean select) {
            ...
            if (isSelected) {
                ...
                ivAdd.setBackgroundResource(R.drawable.item_edit_add_normal);
            }
            invalidate();
        }
        ...
        protected void onLayout(boolean changed, int l, int t, int r, int b) {
            if (itemType == 1) {
                ...
                int crop = 0;
                m_contentView.layout(
                        mItemCloseButtonHorizontalPadding + crop,
                        mItemCloseButtonVerticalPadding + crop, mItemWidth
                                - crop, mItemHeight
                                + mItemCloseButtonVerticalPadding - crop);
            } else {

                m_contentView.layout(-mPadding,
                        mItemCloseButtonVerticalPadding, mItemWidth,
                        mItemHeight + mItemCloseButtonVerticalPadding);
            }
        }
        ...
        public void onClick(View v) {
            mAddItemListener.addItemClick(itemType);
        }
    ...
    public void resetAddItem() {
        for (int nTmp = 0; nTmp < mItemCount; nTmp++) {
            AddViewItem itemAdd = (AddViewItem) this.getChildAt(mItemCount
                    + nTmp);
            if (itemAdd != null) {
                itemAdd.setSelected(false);
            }
        }
    }
    ...
    public void setAddItemListener(AddItemOnClickListener l) {
        mAddItemListener = l;
    }
    ...
    public void hideSort(boolean hide) {
        mHideSort = hide;
    }
}
